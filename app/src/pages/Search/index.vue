              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont"
                      :class="{'icon-Upxiangshang37':isUp,'icon-Downxiangxia':isDown}"></span></a>
                </li>
...
<script>
  ...
  export default {
    ...
    computed: {
      // mapGetters参数的写法：传递的是数组，因为getters计算是没有划分模块【home、search】
    ...
    methods: {
      ...
      removeCategoryName() {
        // 把带给服务器的参数置空
        // 带给服务器的参数是可有可无的，把字段置空也会带给服务器，但是做undefined的去不会
        ...
        this.getData()
        // 地址栏也需要改：进行路由跳转
        // 这里需要删除的就query参数，而不是删除有params的参数
        if (this.$route.params) {
          this.$router.push({ mame: 'search', params: this.$route.params })
        }
      },
      //删除关键字
      removeKeyword() {
