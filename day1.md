...
路由传递参数先关面试题
     1:路由传递参数（对象写法）path是否可以结合params参数一起使用?
        对象的写法可以使name、path的形式，但是，path这种写法不能和params参数一起使用
     2:如何指定params参数可传可不传? 
     比如:配置路由的时候，占位了（params参数)，但是路由跳转的时候就不传递。路径会出现问题
        问题：http://localhost:8080/#/?k=QWE（会没有跳转后的路由地址）
        http://localhost: 8080/#/?k=QwE （跳转前）
        http://localhost:8080/#/search?k=QWE （跳转后正确的）
        解决方法：
        如何指定一个params参数可传可不传：在配置路由的时候，在占位的后面加上一个问号？
     3:params参数可以传递也可以不传递后，但是如果传递是空串，如何解决？
        出现的问题：跟2一样，路径会出现问题
        解决方法：使用undefined  即：params:{keyword:''||undefined}
     4: 路由组件能不能传递props数据?
        可以，三种写法：
            1.布尔值写法：只能通过props传递params参数
                在路由配置中   props:true
            2.对象写法:额外的给路由组织传递一些props
                在路由配置中  props: {a:1,b:2}
            3.函数写法：能通过props传递params参数、query参数
                在路由配置中 props:($route)=>{
